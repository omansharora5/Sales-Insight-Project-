...
#transactions=(transactions['sales_amount']*80)[transactions['currency']=='USD']
#print(transactions.at[135937,'sales_amount'])
#print(transactions.to_string())
#done with cleaning now moving towards analysis

#print(markets.columns)
#print(markets['markets_code'])
...
#transactions_by_date_grp_avg_month_for_graph.index = pd.Categorical(transactions_by_date_grp_avg_month_for_graph.index, categories=new_order , ordered=True)
#transactions_by_date_grp_avg_month_for_graph.sort_index(inplace=True)
#print(type(transactions_by_date_grp_avg_month_for_graph))
#transactions_by_date_grp_avg_month_for_graph.index=transactions_by_date_grp_avg_month_for_graph.index.astype(str)
...
#print(transactions[['customer_code','market_code']].count())
#print((transactions_for_market[transactions_for_market['customer_code']=='cus001']).count())
#transactions_for_market_cus0=(transactions_for_market.groupby(['markets_name'])).size()
#print(transactions_for_market_cus0)
transactions_for_market_cus=(transactions_for_market.groupby('market_code')).size()
#print(transactions_for_market_cus)
#transactions_for_market_cus_2=transactions_for_market_cus0['customer_code'].count()
#print(transactions_for_market_cus_2)
#print(transactions_for_market)
customers_size=transactions_for_market.groupby('customer_code').size()
#print(transactions_for_market)
#customers_size_n1=pd.merge(customers_size,transactions_for_market[['customer_code','merkets_name','profit_margin']],on="customer_code",how='left')
#print(customers_size)
#print(transactions_for_market.describe)

customers_count_in_market = transactions_for_market.groupby('markets_name')['customer_code'].count()

#print(customers_count_in_market)
...
#print((average_transactions_per_customer_in_market))
#print(markets.count())
#print(customers.count())
#print((transactions_for_market.loc[transactions_for_market['markets_name']=='Chennai',['cost_price']]).sum())
#markets_with_more_than_one_customer = customers_count_in_market[customers_count_in_market > 1]
markets_with_unique_customers = transactions_for_market.groupby('markets_name')['customer_code'].nunique()
#print(markets_with_unique_customers)
#print(average_cost_spent_per_market)
...
#print(readable_number_converter(12324385734.493857382))
total_profit_margin=transactions_for_market_grp_total['profit_margin'].sum()
total_revenue_per_market=transactions_for_market_grp_total['sales_amount']
#print(total_revenue_per_market)
...
#transactions_by_date_grp_avg_year.reset_index(inplace=True)
#transactions_by_date_grp_avg_month.reset_index(inplace=True)
#transactions_by_date_grp_avg_month.reset_index(inplace=True)
...
#print(sales_amount_2019)
#print(sales_amount_2020)

own_brand_str=str('Own Brand\r')
distribution_str=str("Distribution\r")
#own_brand_and_distribution_products = products[products['product_type'].isin([own_brand_str,distribution_str])]
...
# Print the counts
#print(own_brand_count['product_type'])
#print("Number of distribution products:", distribution_count)
#print(customers_count_in_market)
#print(type(customers_count_in_market))
#print(customers_count_in_market.index)


#print(transactions_by_date_grp_avg_year[transactions_by_date_grp_avg_year['year']==2019])
...
col1, col2,col3, col5, col6 = st.columns(5,gap='large')
...
#col4.metric(label='Avg Revenue per month each year',value=f"₹{avg_revenue_per_month_1}")

col5.metric(label='Best place',value=(best_place))

col6.metric(label='Best month',value=(best_month))
